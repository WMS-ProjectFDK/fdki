...
$sql = "select aa.id_print,aa.assy_line,cell_type,pallet,sum(bb.qty_label) as qty,tanggal_produksi lot_number,aa.id_print, 
	...
	from  (select  max(tanggal_produksi) tanggal_produksi , id_print, assy_line, cell_type,pallet from ztb_assy_kanban group by id_print, assy_line, cell_type,pallet) aa inner join ztb_wip_bat bb
	on aa.id_print = bb.id_print
	inner join ztb_assy_heating cc
	on aa.id_print = cc.id_print
	where position = 3 and bb.qty_label <> 0 and to_date(upto_date, 'YYYY-MM-DD HH24:MI:SS') between sysdate-4 and sysdate
