...
$sql = "select plt_no,sum( distinct case when plt_no <> ceil(qty / pallet_unit_number) then pi.pallet_unit_number
else case when qty - (floor(qty / pallet_unit_number) * pi.pallet_unit_number) = 0 
      then pi.pallet_unit_number
        else  qty - (floor(qty / pallet_unit_number) * pi.pallet_unit_number) 
        end
end ) QTY_PRINTED
from (
select distinct cast(plt_no as number) plt_no,wo_no from ztb_m_plan where upload = 1 and wo_no = '$wo_no' and plt_no in ($pallet)
union all
select distinct cast(plt_no as number) plt_no, wo_no from ztb_item_book where wo_no = '$wo_no' and plt_no in ($pallet)
)aa
...
$qty = 0;
	...
	while ($row=oci_fetch_array($result)){
		$str .=  ",".rtrim($row[0]) ;
		$qty  = $qty + $row[1];
        // $str = parse_str($str) + rtrim($row[0]) + ",";
	}
	$arrData[$arrNo] = array("PLT_NO"=>rtrim($str),
							 "QTY"=>$qty);
