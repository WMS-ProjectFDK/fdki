...
$qty5 = "select * from (                                   
    select a.id_print, a.id_plan, a.assy_line, a.cell_type, a.pallet, sum(a.qty_act_perpallet) as qty,
        c.id_pallet, b.position, c.upto_date, c.remark, a.tanggal_produksi, a.tanggal_actual
        ...
        group by a.id_print, a.id_plan, a.assy_line, a.cell_type, a.pallet, c.id_pallet, b.position, c.upto_date, c.remark, a.tanggal_produksi, a.tanggal_actual
        order by b.position asc
    ) where (tanggal_actual >= case when position = 1 then (select sysdate-30 from dual) end
            OR
             tanggal_actual <= case when position = 1 then (select sysdate-1 from dual) end                       
            )
      or (to_char(tanggal_actual,'YYYYMM') = case when position = 2 then (select to_char(sysdate-1,'YYYYMM') from dual) end)";
